...
-(void)getButtonsWidthWithTitles{
    ...
    for (NSString * str  in self.titleArr) {
        ...
        CGFloat Width = UISCREENWIDTH/5;
        NSLog(@"%@",str);
        [self.itemsWidth addObject:@(Width)];

        CGFloat LabelX =  CGRectGetMaxX(lastlabel.frame) ;
        UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(LabelX, 13, Width, 20)];
...
- (void)adjustUIWhenBtnOnClickWithAnimate:(BOOL)animated taped:(BOOL)taped {
    ...
    [UIView animateWithDuration:animatedTime animations:^{
        oldlabel.textColor = AppThemeColor;
...
- (void)adjustTitleOffSetToCurrentIndex:(NSInteger)currentIndex {
    ...
    for (UILabel *titleView in self.titleViews) {
        if (index != currentIndex) {
            ...
            titleView.textColor = AppThemeColor;
